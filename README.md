## Diffusion VLA 추론 영상
https://drive.google.com/file/d/1wp5mqv8aumPWgnGYO7Gtsdia3r3qO_tQ/view?usp=drive_link 
...
## 아쉬운 점
현재 파이프라인은 의자에 대한 접근 동작 자체는 이전보다 개선되었지만, 최종적으로 안정적인 의자 파지 성공까지는 도달하지 못했습니다.
특히 policy가 의자 근처까지 접근하는 품질은 향상되었으나, 실제 grasp 성공률 측면에서는 아직 한계가 남아 있습니다.

가장 큰 원인 중 하나는 데이터 다양성 부족이라고 판단하고 있습니다.
현재 학습 데이터는 대부분 동일한 환경, 유사한 시점, 비슷한 조명 조건, 그리고 거의 같은 형태의 의자 집기 장면으로 구성되어 있어, 모델이 다양한 상황에 일반화하기 어려웠습니다.
즉, policy가 특정한 장면 패턴에는 적응했지만, 실제 실행 과정에서 발생하는 미세한 위치 차이, 접근 각도 변화, depth 오차, 시점 변화 등에 충분히 강인하지 못했습니다.

또한 grasp 직전의 세밀한 조정이 필요한 상황에서, 데이터가 반복적으로 비슷한 motion pattern에 편향되어 있었기 때문에,
모델이 “의자를 어떻게 집어야 하는가”를 폭넓게 학습하기보다는 특정한 예시를 제한적으로 모사하는 경향을 보였습니다.

향후에는 다음과 같은 방향으로 보완할 계획입니다.

- 다양한 의자 위치, 자세, 조명, 배경 조건을 포함한 데이터 추가 수집
- 카메라 시점 변화 및 depth noise가 포함된 장면 확장
- grasp 성공/실패 사례를 모두 포함한 데이터 구성
- 접근 단계뿐 아니라 grasp 직전 정렬 및 파지 순간에 대한 데이터 다양성 확보
- 외부 카메라 기반 평가 또는 reward 설계를 통한 정책 개선
